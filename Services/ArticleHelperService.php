...
class ArticleHelperService
{
    ...
    public function getArticleUrl($article, $side = self::FRONT_SIDE, array $params = array())
    {
        ...
        return $this->router
            ->generate('newscoop_tageswochemobileplugin_articles_list', $params);
    ...
    {
        return '/' . Manager::getWebcoder('')->encode($article->getNumber());
    ...
    public function formatArticle($article)
    {
        ...
        $thing = $renderSlideshowHelper->direct($article->getNumber());
        echo '<pre>$thing:'. nl2br(print_r($thing, true)).'</pre><hr>'; exit;
