...
class GenerateOfflineIssueCommand extends ContainerAwareCommand
{
    ...
    protected function execute($input, $output)
    {
        if ($input->getArgument('issue') === 'all') {
            ...
            $issue = $this->getContainer()->get('newscoop_tageswochemobile_plugin.mobile.issue')->find($input->getArgument('issue'));
            if ($issue !== null) {
                $this->generateIssue($issue);
            } else {
    ...
    {
        try {
            $this->getContainer()->get('newscoop_tageswochemobile_plugin.mobile.issue.offline')->generateIssue($issue);
        } catch (\Exception $e) {
